		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="switch" @click="show_list_flight_information = !show_list_flight_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_flight_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--航班信息推荐列表组件 -->
		                <list_flight_information v-if="show_list_flight_information" :list="list_flight_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_flight_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_flight_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_flight_information" :key="i" @click="to_details('flight_information',o,'flight_information_id')">
		                            <td v-for="(oj,n) in fields_flight_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		methods: {
			...
			get_flight_information() {
				let url = "~/api/flight_information/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
		...
		computed: {
		    list_table_flight_information() {
		        var list = this.list_flight_information;
		        var list_table = [];
