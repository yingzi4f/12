<template>
	<div class="diy_list page_flight_information" id="flight_information_list">
		<div class="warp">
			<div class="container diy_list_container">
				<div class="diy_list_title">
					<div class="col">
						<span class="title">航班信息列表</span>
					</div>
				</div>
				<div class="row diy_list_search">
					<div class="col">
						<!-- 搜索栏 -->
						<div class="view">							
							<span class="diy_list_search_title">关键字搜索：</span>
							<b-form-input size="sm" class="mr-sm-2" placeholder="出发地搜索" v-model="query['place_of_departure']" />
							<b-form-input size="sm" class="mr-sm-2" placeholder="目的地搜索" v-model="query['destination']" />
							<b-form-input size="sm" class="mr-sm-2" placeholder="出发时间搜索" v-model="query['departure_time']" />
							<b-form-input size="sm" class="mr-sm-2" placeholder="航班类型搜索" v-model="query['flight_type']" />
							<b-button size="sm" @click="search()" >
								<b-icon icon="search"/>
							</b-button>
						</div>
						<!-- /搜索栏 -->
					</div>
				</div>
				<div class="diy_list_select_box">
					<span class="diy_list_select_title">下拉搜索：</span>
						<div class="diy_list_dropdown_box">
						<div class="col">
							<!-- 筛选 -->
							<div class="view">
								<b-dropdown :text="title" variant="outline-dark" left>
									<b-dropdown-item @click="filter_set('全部','flight_type')">全部</b-dropdown-item>
										<b-dropdown-item v-for="(o,i) in list_flight_type" @click="filter_set(o,'flight_type')" >
										{{ o }}
										</b-dropdown-item>
								</b-dropdown>
							</div>
							<!-- /筛选 -->
						</div>
					</div>
					<div class="diy_list_sort_box">
						<div class="col">
							<!-- 排序 -->
							<div class="view">
								<b-dropdown text="排序" variant="outline-dark" left>
										<b-dropdown-item v-for="(o, i) in list_sort" :key="i" @click="set_sort(o)" >
												{{ o.name }}
										</b-dropdown-item>
								</b-dropdown>
							</div>
							<!--/排序 -->
						</div>
					</div>
				</div>
				<div class="row diy_list_box">
					<div class="col">
						<!-- 列表 -->
						<list_flight_information :list="list" />
						<!-- /列表 -->
					</div>
				</div>
				<div class="row diy_list_page_box">
					<div class="col overflow-auto flex_cc">
						<!-- 分页器 -->
<!--						<diy_pager v-model="query['page']" :size="query['size']" :count="count" v-on:toPage="toPage" v-on:toSize="toSize" ></diy_pager>-->
            <b-pagination
                v-model="query.page"
                :total-rows="count"
                :per-page="query.size"
                @change="goToPage"
            />
						<!-- /分页器 -->
					</div>
				</div>
			</div>
		</div>
	</div>
</template>

<script>
	import list_flight_information from "@/components/diy/list_flight_information.vue";
	import diy_pager from "@/components/diy/diy_pager";
	import mixin from "@/mixins/page.js";

	export default {
		mixins: [mixin],
		components: {
			diy_pager,
			list_flight_information
		},
		data() {
			return {
				url_get_list: "~/api/flight_information/get_list?like=0",

				// 查询条件
				query: {
					keyword: "",
					page: 1,
					size: 10,
					"place_of_departure": "", // 出发地
					"destination": "", // 目的地
					"departure_time": "", // 出发时间
					"flight_type": "", // 航班类型
				},

				// 排序内容
				list_sort: [{
						name: "创建时间从高到低",
						value: "create_time desc",
					},
					{
						name: "创建时间从低到高",
						value: "create_time asc",
					},
					{
						name: "更新时间从高到低",
						value: "update_time desc",
					},
					{
						name: "更新时间从低到高",
						value: "update_time asc",
					},
					{
						name: "出发地正序",
						value: "place_of_departure asc",
					},
					{
						name: "出发地倒序",
						value: "place_of_departure desc",
					},
					{
						name: "目的地正序",
						value: "destination asc",
					},
					{
						name: "目的地倒序",
						value: "destination desc",
					},
					{
						name: "出发时间正序",
						value: "departure_time asc",
					},
					{
						name: "出发时间倒序",
						value: "departure_time desc",
					},
					{
						name: "航班类型正序",
						value: "flight_type asc",
					},
					{
						name: "航班类型倒序",
						value: "flight_type desc",
					},
				],
				// 航班类型列表
				"list_flight_type": ['国内','国际'],
				// 座位类型列表
				"list_seat_type": ['经济舱','头等舱'],
				// 客机编号列表
				"list_passenger_plane_number": [],
				// 客机型号列表
				"list_aircraft_model": [],
			}
		},
		methods: {
      get_list_before(param) {
      },
			/**
			 * 筛选选择
			 */
			filter_set(o,key) {
			    if (o == "全部") {
			        this.query[key] = "";
			    } else {
			        this.query[key] = o;
			    }
			    this.search();
			},

			/**
			 * 排序
			 */
			set_sort(o) {
			    this.query.orderby = o.value;
			    this.search();
			},
			/**
			 * 获取客机编号列表
			 */
			async get_list_passenger_plane_number() {
				var json = await this.$get("~/api/passenger_plane_management/get_list?");
				if (json.result) {
					this.list_passenger_plane_number = json.result.list;
				} else if (json.error) {
					console.log(json.error);
				}
			},
			/**
			 * 获取客机型号列表
			 */
			async get_list_aircraft_model() {
				var json = await this.$get("~/api/passenger_plane_management/get_list?");
				if (json.result) {
					this.list_aircraft_model = json.result.list;
				} else if (json.error) {
					console.log(json.error);
				}
			},

			/**
			 * 筛选
			 */
			filter_flight_type(o) {
				if (o == "全部") {
					this.query["flight_type"] = "";
				} else {
					this.query["flight_type"] = o;
				}
				this.search();
			},

			/**
			 * 重置
			 */
			reset() {
				this.query.place_of_departure = ""
				this.query.destination = ""
				this.query.departure_time = ""
				this.query.flight_type = ""
				this.search();
			},

			// 返回条数
			toSize(i){
				this.query.size = i;
				this.first();
			},

			// 返回页数
			toPage(i){
				this.query.page = i;
				this.first();
			},

      goToPage(v){
        this.query.page = v;
        this.goToNew(v)
      },
		},
		computed: {
		},
		created() {
			/**
			 * 获取客机编号列表
			 */
			this.get_list_passenger_plane_number();
			/**
			 * 获取客机型号列表
			 */
			this.get_list_aircraft_model();
		}
	}
</script>

<style>
</style>
