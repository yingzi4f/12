<template>
	<div class="diy_home diy_list diy_visa_change_application" id="diy_visa_change_application_list">
		<!-- 列表 -->
		<div class="diy_view_list list list-x" v-if="show">
			<router-link class="diy_card goods diy_list_box_wrap" v-for="(o, i) in list" :key="i"
				:to="'/visa_change_application/details?visa_change_application_id=' + o['visa_change_application_id']">
				<!-- 图片 -->
				<div class="diy_list_img_box" v-if="imgList.length" >
					<div class="diy_row" v-for="(item,index) in imgList" :key="item+index" v-show="$check_field('get',item.name,'/visa_change_application/details') && +item.is_img_list">
				...
				<div class="diy_list_item_box">
					<div class="diy_list_item_content" v-for="(item,index) in showItemList" :key="item+index">
						<div class="diy_row" :class="{[item.name]:true}" v-if="$check_field('get',item.name,'/visa_change_application/details') && +item.is_img_list">
							<div class="diy_title">
								<span>{{item.title}}:</span>
							</div>
		...
		<div class="diy_view_table" v-else>
			<table class="diy_table">
				<tr class="diy_row">
					...
					<th class="diy_title" v-if="$check_field('get','number_of_purchased_sheets')">购买张数
					</th>
					<th class="diy_title" v-if="$check_field('get','total_amount')">总金额
					</th>
					<th class="diy_title" v-if="$check_field('get','user_account')">用户账号
					</th>
					<th class="diy_title" v-if="$check_field('get','application_time')">申请时间
					</th>
					<th class="diy_title" v-if="$check_field('get','change_destination')">改签目的地
					</th>
					<th class="diy_title" v-if="$check_field('get','number_of_changed_signatures')">改签张数
					</th>
					<th class="diy_title" v-if="$check_field('get','change_departure_time')">改签出发时间
					</th>
					<th class="diy_title" v-if="$check_field('get','reason_for_change')">改签原因
					</th>
				</tr>
				<tr class="diy_row" v-for="(o,i) in list" :key="o+i">
					...
					<td class="diy_field diy_text" v-if="$check_field('get','departure_time')">
						<span>
							{{ o["departure_time"] }}
						</span>
					</td>
					...
					<td class="diy_field diy_text" v-if="$check_field('get','number_of_purchased_sheets')">
						<span>
							{{ o["number_of_purchased_sheets"] }}
						</span>
					</td>
					...
					<td class="diy_field diy_text" v-if="$check_field('get','number_of_changed_signatures')">
						<span>
							{{ o["number_of_changed_signatures"] }}
						</span>
					</td>
					<td class="diy_field diy_text" v-if="$check_field('get','change_departure_time')">
						<span>
							{{ o["change_departure_time"] }}
						</span>
					</td>
					<td class="diy_field diy_text" v-if="$check_field('get','reason_for_change')">
						<span>
							{{ o["reason_for_change"] }}
						</span>
					</td>
		...
		data() {
			return {
						...
						itemList: [
								...
								{
									title: "购买张数",
									name: "number_of_purchased_sheets",
									type: "文本",
									is_img_list: "0"
								},
								{
								...
								{
									title: "改签张数",
									name: "number_of_changed_signatures",
									type: "文本",
									is_img_list: "0"
								},
								{
									title: "改签出发时间",
									name: "change_departure_time",
									type: "文本",
									is_img_list: "0"
								},
						],
