<template>
	<div class="diy_details_box diy_div_balance_recharge">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "balance_recharge",
					source_field: "balance_recharge_id",
					source_id: this.obj["balance_recharge_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/balance_recharge/set?balance_recharge_id=' + _this.obj["balance_recharge_id"], {
								praise_len
							}, (res) => {
								if(res.result){
