<template>
	<div class="diy_details_box diy_div_visa_change_application">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "visa_change_application_id",
					source_id: this.obj["visa_change_application_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/visa_change_application/set?visa_change_application_id=' + _this.obj["visa_change_application_id"], {
								praise_len
							}, (res) => {
								if(res.result){
