<template>
	<div class="diy_details_box diy_div_order_refund">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "order_refund_id",
					source_id: this.obj["order_refund_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/order_refund/set?order_refund_id=' + _this.obj["order_refund_id"], {
								praise_len
							}, (res) => {
								if(res.result){
