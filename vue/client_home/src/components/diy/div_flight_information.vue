<template>
	<div class="diy_details_box diy_div_flight_information">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/order_center/edit','add')" @click="to_form('/order_center/edit')" >购票 </button>
							</div>
						<!-- 富文本 -->
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/flight_information/set?flight_information_id=' + _this.obj["flight_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
