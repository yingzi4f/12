<template>
	<div class="diy_details_box diy_div_rescheduling_payment">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "rescheduling_payment_id",
					source_id: this.obj["rescheduling_payment_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/rescheduling_payment/set?rescheduling_payment_id=' + _this.obj["rescheduling_payment_id"], {
								praise_len
							}, (res) => {
								if(res.result){
