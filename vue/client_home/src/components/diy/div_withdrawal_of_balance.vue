<template>
	<div class="diy_details_box diy_div_withdrawal_of_balance">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "withdrawal_of_balance",
					source_field: "withdrawal_of_balance_id",
					source_id: this.obj["withdrawal_of_balance_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/withdrawal_of_balance/set?withdrawal_of_balance_id=' + _this.obj["withdrawal_of_balance_id"], {
								praise_len
							}, (res) => {
								if(res.result){
