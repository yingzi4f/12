			...
			<el-col v-if="user_group === '管理员' || $check_field('get','account_number') || $check_field('add','account_number') || $check_field('set','account_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="账号编号" prop="account_number">
					<el-input id="account_number" v-model="form['account_number']" placeholder="请输入账号编号"
							  v-if="user_group === '管理员' || (form['balance_recharge_id'] && $check_field('set','account_number')) || (!form['balance_recharge_id'] && $check_field('add','account_number'))" :disabled="disabledObj['account_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','recharge_amount') || $check_field('add','recharge_amount') || $check_field('set','recharge_amount')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="充值金额" prop="recharge_amount">
					<el-input-number id="recharge_amount" v-model.number="form['recharge_amount']"
						v-if="user_group === '管理员' || (form['balance_recharge_id'] && $check_field('set','recharge_amount')) || (!form['balance_recharge_id'] && $check_field('add','recharge_amount'))" :disabled="disabledObj['recharge_amount_isDisabled']"></el-input-number>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','user_account') || $check_field('add','user_account') || $check_field('set','user_account')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="用户账号" prop="user_account">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['balance_recharge_id'] && $check_field('set','user_account')) || (!form['balance_recharge_id'] && $check_field('add','user_account'))" :disabled="disabledObj['user_account_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','user_account')">{{form['user_account']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['balance_recharge_id'] && $check_field('set','user_account')) || (!form['balance_recharge_id'] && $check_field('add','user_account'))" id="user_account" v-model="form['user_account']" :disabled="disabledObj['user_account_isDisabled']">
	...
	export default {
		...
		data() {
			return {
				...
				url_upload: "~/api/balance_recharge/upload?",
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/balance_recharge/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/balance_recharge/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/balance_recharge/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/balance_recharge/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/balance_recharge/view','get');
